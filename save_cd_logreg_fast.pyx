...
cdef double step_size(double[::1] y, double * X_j, double[::1] Xbeta,
                      double[::1] Xbeta_next, double beta_j, double beta_prox,
                      double delta, double p_obj, double norm1_beta_next,
                      double XTR_j, double lambda_, int n_samples) nogil:
    cdef:
    ...
    while decrease == 0:
        ...
        if beta_next_j == beta_next_old_j:
            break
        ...
        for i in range(n_samples):
            Xbeta_next[i] += X_j[i] * (beta_next_j - beta_next_old_j)
...
def cd_logreg(double[::1, :] X, double[::1] y, double[::1] beta,
    ...
    with nogil:
        ...
        for n_iters in range(max_iter):
                        ...
                        for j in range(n_features):
                            ...
                            if fabs(XTR[j] / dual_scale) + r_normX_j < 1.:
                                ...
                                if beta[j] != 0:
                                    ...
                                    p_obj = -yTXbeta + log_term + lambda_ * norm1_beta
                                    beta[j] = 0.
            ...
            for j in range(n_features):
                ...
                for i in range(n_samples):
                    exp_Xbeta_i = exp(Xbeta[i])
                    Hessian[j] += X[i, j] ** 2 * exp_Xbeta_i / (1. + exp_Xbeta_i) ** 2

                L_j = fmax(Hessian[j], 1e-12)
                mu = lambda_ / L_j
                beta_old_j = beta[j]
                XTR[j] = ddot(& n_samples, & X[0, j], & inc, & residual[0], & inc)
                beta_prox_j = ST(mu, beta[j] + XTR[j] / L_j)
                # line search
                delta_j = beta_prox_j - beta[j]
                alpha_j = step_size(y, & X[0, j], Xbeta, Xbeta_next,
                                    beta[j], beta_prox_j, delta_j, p_obj,
                                    norm1_beta, XTR[j], lambda_, n_samples,)
                # Update beta[j]
                ...
                if beta[j] != beta_old_j:
                    ...
                    for i in range(n_samples):
                        Xbeta[i] += X[i, j] * (beta[j] - beta_old_j)
                        exp_Xbeta_i = exp(Xbeta[i])
                        residual[i] = y[i] - exp_Xbeta_i / (1. + exp_Xbeta_i)
                        # update p_obj
                        yTXbeta += y[i] * Xbeta[i]
